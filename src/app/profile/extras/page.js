...
export default function Extras() {
	...
	const handleNewExtra = async e => {
		e.preventDefault()

		const data = { name: newExtra }
		if (editedExtra) {
			data._id = editedExtra._id
		}
		const res = await fetch('/api/extras', {
			method: editedExtra ? 'PUT' : 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify(data),
		})
		const { status } = res
		if (res.ok) {
			if (editedExtra) toast.success('Запись изменена')
			else toast.success('Запись создана')
			fetchExtras()
			setNewExtra('')
			setEditedExtra(null)
		} else {
	...
	const handleDeleteClick = async _id => {
		const res = await fetch('/api/extras?_id=' + _id, {
			method: 'DELETE',
		})
			...
			<div className='flex-grow'>
				...
				>
					<div className='relative z-0 grow mb-5 group mr-6'>
						...
						>
							{editedExtra ? 'Редактирование записи' : 'Новая запись'}
							{editedExtra && (
								<>
									: <b>{editedExtra.name}</b>
								</>
							)}
						</label>
					</div>
